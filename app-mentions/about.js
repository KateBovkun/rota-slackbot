  ...
  try {
    ...
    if (utils.rotationInList(rotation, ec.rotaList)) {
      ...
      const result = await app.client.chat.postMessage(
        utils.msgConfigThread(ec.botToken, ec.channelID, ec.ts, msgText.aboutReport(rotation, rotationObj))
      );
      if (!!ec.sentByUserID && ec.sentByUserID !== 'USLACKBOT') {
        ...
        const ephStaffResult = await app.client.chat.postEphemeral(
          utils.msgConfigEph(ec.botToken, ec.channelID, ec.sentByUserID, ec.ts, msgText.aboutStaffEph(rotation, rotationObj.staff))
        );
      }
    } else {
      // If rotation doesn't exist, send message saying nothing changed
      const result = await app.client.chat.postMessage(
        utils.msgConfigThread(ec.botToken, ec.channelID, ec.ts, msgText.aboutError(rotation))
      );
    }
  }
  catch (err) {
