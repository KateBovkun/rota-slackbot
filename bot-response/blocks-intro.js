/*------------------
   BLOCKS: INTRO
------------------*/
const introBlocks = [
  {
    "type": "section",
    "text": {
      "type": "mrkdwn",
      "text": ":wave: Hi there! I'm your friendly *Rotation Bot* :arrows_counterclockwise::robot_face: My role is to help track many different rotations across teams."
    }
  },
  {
    "type": "section",
    "text": {
      "type": "mrkdwn",
      "text": ':speech_balloon: *If you need help with a rotation, post a message and mention `@{usergroup} [your message]`*. The person on duty for the rotation will be notified!'
    }
  },
  {
    "type": "divider"
  }
];

module.exports = introBlocks;