...
const msgText = {
  newConfirm: (rotation) => {
    return ':sparkles: The *<#' + rotation + '>* rotation has been created. You can now assign someone to be goalie for this rotation or add a staff list. Use `@rota help` to learn more.';
  },
  newError: (rotation) => {
    return ':mag_right: The *<#' + rotation + '>* rotation already exists. You can assign someone to be goalie with `@rota assign [@user]`.';
  },
  ...
  descError: (rotation) => {
    return ':shrug: I couldn\'t update the description for <#' + rotation + '> because it does not exist. When you create a new rotation, you can include the description: `@rota new [description]`.';
  },
  staffConfirm: (rotation) => {
    return ':busts_in_silhouette: The *<#' + rotation + '>* rotation staff list has been saved! You can now use `@rota assign next` to rotate assignments.\nWhen using `next`, if the currently goalie person is not in the staff list, the assignment will default to the _first person_ in the rotation.\n_(Note: I remove duplicates. If you want someone to do additional shifts, you\'ll need to do a username assignment.)_';
  },
  staffEmpty: () => {
  ...
  staffError: (rotation) => {
    return ':shrug: I couldn\'t save staff for <#' + rotation + '> because it does not exist. To create this rotation, first tell me `@rota new [description]`, _then_ set up staffing.';
  },
  ...
  resetStaffError: (rotation) => {
    return ':shrug: I couldn\'t reset the staff list for <#' + rotation + '> because it does not exist. To create this rotation, first tell me `@rota new [description]`. It will initialize with no staff by default.';
  },
  ...
  aboutError: (rotation) => {
    return ':shrug: I couldn\'t get any info about <#' + rotation + '> because it does not exist. To create it, use `@rota new [description]` in the <#' + rotation + '> channel.';
  },
  ...
  assignNextError: (rotation) => {
    return ':shrug: I couldn\'t assign the next person in the *<#' + rotation + '>* rotation because there is nobody in the rotation staff list.\nTo set staff, use `@rota staff [@user1 @user2 @user3]`.\nThen you can use `@rota assign next [optional handoff message]` to rotate to the next user on staff.\nIf you don\'t want to use a staff list, assign someone directly by username: `@rota assign [@user] [optional handoff message]`.';
  },
  assignError: (rotation) => {
    return ':shrug: I couldn\'t complete this assignment because <#' + rotation + '> does not exist. To create it, use `@rota new [description]`.';
  },
  listReport: (list) => {
    let msgStr = '';
  ...
  listEmpty: () => {
    return ':clipboard: There are no rotations saved right now. To create one, tell me `@rota new [description]`.';
  },
  ...
  nobodyAssigned: (rotation) => {
    return ':ghost: Nobody is currently assigned to the *<#' + rotation + '>* rotation. To assign someone, use `@rota assign [@user] [optional handoff message]` or assign the next person in the rotation staff list with `@rota assign next [optional handoff message]` in the <#' + rotation + '> channel.';
  },
  whoError: (rotation) => {
    return ':shrug: I couldn\'t find any goalie because <#' + rotation + '> does not exist. To create it, use `@rota new [description]` in the <#' + rotation + '> channel. You can then assign someone using `@rota assign [@user] [optional handoff message]`.';
  },
  ...
  unassignError: (rotation) => {
    return ':shrug: I couldn\'t clear this assignment because <#' + rotation + '> does not exist. To create it, use `@rota new [description]`.';
  },
  confirmChannelMsg: (rotation, sentByUserID) => {
    if (sentByUserID) {
      return `:speech_balloon: The goalie for *<#${rotation}>* has been notified about <@${sentByUserID}>'s message.`;
    } else {
  ...
  msgError: (rotation) => {
    return ':shrug: I couldn\'t tell anyone about the message because <#' + rotation + '> does not exist. To create it, use `@rota new [description]` in the <#' + rotation + '> channel.';
  },
